...
- [x] Script di import idempotente: posso rieseguirlo e aggiorna invece di duplicare
  - _Note implementative: aggiunto `scripts/bootstrap-sherdan.ts` con script `pnpm db:bootstrap:sherdan`, piu' piano puro testabile `buildSherdanBootstrapPlan` in `src/lib/import/sherdan-bootstrap-plan.ts`. Primo passaggio idempotente: assicura la campagna "Sherdan", crea/aggiorna 153 entita' (PC, NPC, fazioni, luogotenenti, lore), 81 identita', 56 segreti, 6 sessioni, 10 plot thread, 47 rule documents e i `pc_hooks` risolvibili. Le relazioni name-based restano volutamente `deferredLinks` (45) per il task successivo di cross-reference; 32 `pc_hooks` restano unresolved perche' usano etichette aggregate/non mappabili a un singolo PG. Verifica live con Docker/Postgres: prima esecuzione crea/aggiorna, seconda esecuzione non duplica (`entitiesCreated: 0`, `entitiesUpdated: 153`, `identitiesUpdated: 81`, `secretsSkipped: 56`, `sessionsUpdated: 6`, `plotThreadsUpdated: 10`, `ruleDocumentsUpdated: 47`, `pcHooksExisting: 26`)._
...
- [x] Report di import: cosa è stato creato, cosa è stato skippato, warning su strutture non riconosciute
  - _Note implementative: aggiunto `pnpm db:report:sherdan` (`scripts/report-sherdan-import.ts`) e builder puro `buildSherdanImportReport`/`renderSherdanImportReportMarkdown` in `src/lib/import/sherdan-import-report.ts`. Il report viene generato in `docs/sherdan-import-report.md` e confronta piano parser vs DB: 153 righe entity pianificate, 151 record unici/persistiti, 81 identita', 56 segreti, 26 `pc_hooks` risolti, 45 `entity_links`, 6 sessioni, 10 plot thread, 47 rule documents e 151/151 embedding presenti. Evidenzia anche skipped/unresolved: 2 righe entity duplicate del piano (`npc:Razza:`, `npc:Aspetto:`), 3 entity link unresolved, 32 PC hook unresolved e 107 warning parser._

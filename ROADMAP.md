...
- [ ] LLM client wrapper: interfaccia `complete(prompt, options)` + `complete_structured(prompt, schema)`, implementazione Anthropic
- [ ] Config management: `.env`, dotenv-safe, type-safe config object
- [ ] Logging strutturato: pino (TS) o structlog (Python)
- [ ] Test setup: vitest/jest (TS) o pytest (Python)
- [ ] Layout principale: shell con sidebar + main content (placeholder)
- [ ] Routing base: home, campaigns list, campaign view (placeholder)
- [ ] CI minima: lint + test su push (GitHub Actions)
...
- [ ] API routes CRUD per `entity_secrets` (filtro per layer)
- [ ] API routes CRUD per `pc_hooks`
- [ ] Endpoint `GET /entities/:id/backlinks`
- [ ] Endpoint `GET /search?q=...` (FTS su nome + description + public_description)
- [ ] Validation server-side delle `properties` JSONB per ogni `type`
